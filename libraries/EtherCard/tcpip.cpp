...
// LK changed datalen from byte to word and removed 220 limit
...
// LK added broadcast function for UDP

void EtherCard::sendUdpBroadcast (char *data, word datalen, word sport, word dport) {
  udpPrepare(sport, EtherCard::myip, dport);
  if (datalen>1458)
    datalen = 1458;
  EtherCard::copyMac(gPB + ETH_DST_MAC, allOnes);
  gPB[IP_DST_P+3]=0xff; // Broadcast IP (Assumes netmask 255.255.255.0)
  memcpy(gPB + UDP_DATA_P, data, datalen);
  udpTransmit(datalen);
}
