...
static byte wwwport_h=0;  // Note: never use same as TCPCLIENT_SRC_PORT_H
...
static byte tcp_fd=0; // a file descriptor, will be encoded into the port
static byte tcp_client_state=0;
// TCP client Destination port
static byte tcp_client_port_h=0;
static byte tcp_client_port_l=0;
// This function will be called if we ever get a result back from the
// TCP connection to the sever:
// close_connection= your_client_tcp_result_callback(byte fd, byte statuscode,uint16_t data_start_pos_in_buf, uint16_t len_of_data){...your code}
// statuscode=0 means the buffer has valid data
static byte (*client_tcp_result_callback)(byte,byte,uint16_t,uint16_t);
// len_of_data_filled_in=your_client_tcp_datafill_callback(byte fd){...your code}
...
#define TCP_client 1
#endif
static byte www_fd=0;
static byte browsertype=0; // 0 = get, 1 = post
...
static byte *bufptr=0; // ugly workaround for backward compatibility
#endif
...
static volatile byte waitgwmac=WGW_INITIAL_ARP;
...
uint16_t checksum(byte *buf, uint16_t len,byte type){
        // type 0=ip , icmp
        //      1=udp
        //      2=tcp
        uint32_t sum = 0;
...
byte check_ip_message_is_from(byte *buf,byte *ip)
{
        byte i=0;
        while(i<4){
...
byte eth_type_is_arp_and_my_ip(byte *buf,uint16_t len){
        byte i=0;
        //  
        if (len<41){
                return(0);
...
byte eth_type_is_ip_and_my_ip(byte *buf,uint16_t len){
        byte i=0;
...
void make_eth_ip_new(byte *buf, byte* dst_mac)
{
        byte i=0;
        //
        //copy the destination mac from the source and fill my mac into src
...
byte eth_type_is_arp_reply(byte *buf){
...
byte eth_type_is_arp_req(byte *buf){
...
// make a return ip header from a received ip packet
void make_ip_tcp_new(byte *buf, uint16_t len,byte *dst_ip)
{
    byte i=0;
...
void step_seq(byte *buf,uint16_t rel_ack_num,byte cp_seq)
{
        byte i;
        byte tseq;
        i=4;
...
void make_tcphead(byte *buf,uint16_t rel_ack_num,byte cp_seq)
{
        byte i;
        // copy ports:
...
uint16_t get_tcp_data_len(byte *buf)
{
        int16_t i;
...
uint16_t fill_tcp_data_len(byte *buf,uint16_t pos, const byte *s, uint16_t len)
{
        // fill in tcp data at position pos
        //
        // with no options the data starts after the checksum + 2 more bytes (urgent ptr)
...
void make_tcp_ack_from_any(byte *buf,int16_t datlentoack,byte addflags)
{
        uint16_t j;
        make_eth(buf);
        // fill the header:
...
void make_tcp_ack_with_data_noflags(byte *buf,uint16_t dlen)
{
        uint16_t j;
        // total length field in the IP header must be set:
...
// length and checksum
// Used?
void make_tcp_ack_with_data(byte *buf,uint16_t dlen)
{
        uint16_t j;
        // fill the header:
...
void fill_buf_p(byte *buf,uint16_t len, const prog_char *progmem_s)
{
        while (len){
...
uint16_t i;
	...
	for (i=0; i<6; i++) { // Override header for broadcast
...
byte client_store_gw_mac(byte *buf)
{
        byte i=0;
        while(i<4){
...
// Used?
/*
void client_set_wwwip(byte *wwwipaddr)
{
        byte i=0;
        while(i<4){
                wwwip[i]=wwwipaddr[i];
...
void client_syn(byte *buf,byte srcport,byte dstport_h,byte dstport_l)
{
        uint16_t ck;
        byte i=0;
...
// XXXXXXXXXXX
uint16_t build_tcp_data(byte *buf, uint16_t srcPort )
{
        uint16_t ck;
        byte i=0;
        // -- make the main part of the eth/IP/tcp header:
...
// Send the data
void send_tcp_data(byte *buf,uint16_t dlen)
{
        uint16_t j;
        // fill the header:
        // This code requires that we send only one data packet
...
//
// byte your_client_tcp_result_callback(byte fd, byte statuscode,uint16_t data_start_pos_in_buf, uint16_t len_of_data){...your code;return(close_tcp_session);}
//
...
// uint16_t your_client_tcp_datafill_callback(byte fd){...your code;return(len_of_data_filled_in);}
//
// Now call: 
...
#if defined (WWW_client) 
uint16_t www_client_internal_datafill_callback(byte fd){
        char strbuf[5];
        uint16_t len=0;
        if (fd==www_fd){
                if (browsertype==0){
...
byte www_client_internal_result_callback(byte fd, byte statuscode, uint16_t datapos, uint16_t len_of_data){
        if (fd!=www_fd){
...
//
// Declare a callback function: void browserresult(byte statuscode){...your code}
...
#else
void client_browse_url(char *urlbuf, char *urlbuf_varpart, char *hoststr, void (*callback)(byte,uint16_t,uint16_t))
#endif
{
